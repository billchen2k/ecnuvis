...
export default function PublicationFC(props: IPublicationProps) {
  const [publication, setPublication] = React.useState<Publication[]>([]);
  const [showAbstract, setShowAbstract] = React.useState<boolean[]>([]);
  const [pagerStatus, setPagerStatus] = React.useState<string | undefined>(
      undefined
  );
  ...
  return (
    <div>
      <div className="flex flex-row gap-8 mb-4 items-center textcolor-body justify-center max-md:flex-col max-md:items-start max-md:gap-3">
        <YearlyPager
          fullData={allPublications}
          maximumYears={8}
          showAll
          customStatus={pagerStatus}
          ...
          onSelectionChange={(items, year) => {
            ...
            items.sort((a, b) => {
              return (
                new Date(b.dateCalc).getTime() - new Date(a.dateCalc).getTime()
              );
            });
            setPagerStatus(undefined);
            setPublication(items);
      ...
      <div className="flex flex-col gap-4">
        {[...publication].map((item, index) => (
          <PublicationItem
            publication={item}
            key={item._id}
            showAbstract={showAbstract[index] || false}
            toggleShowAbstract={() => {
              const newAbstractStatus = new Array(publication.length).fill(
                  false
              );
